...
Interactive mode also asks whether to set the generated `<Scope>Permission` enum in `config/access.php` and whether to generate invitation mail helpers. The permission enum prompt defaults to yes when `permission_enum` is still `null`; pass `--no-permission-enum` to leave it unchanged. Pass `--notifications` in non-interactive runs to generate the mail notification and controller helper methods:
...
Use `--no-permission-enum` when you intentionally want `access:scope` to generate `CompanyPermission` for reference but keep `permission_enum` pointed at another enum, such as the `Permission` enum generated by `access:install --enum`.
